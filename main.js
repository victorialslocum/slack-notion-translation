...
const returnEmojiHTML = (slackEmoji) => {
  var emojiHTML = "";
  for (var key in emojis) {
    if (key == slackEmoji) {
      emojiHTML = emojis[key];
    }
  }
  return emojiHTML;
};

var newLineSplit = slackExample.split("\n");
newLineSplit = newLineSplit.filter(Boolean);

// var regex = new RegExp(/[\<\>]/);

// var splitSlack = slackExample.split(regex);

console.log(newLineSplit);

const changeEmoji = (str) => {
  return str.replace(str, function(match, dec) {
  ...
  splitString.forEach((word) => {
    for (var key in emojis) {
      if (word.search(":" + key + ":") != -1) {
        ...
        string = string.replace(regexKey, String.fromCharCode(emojis[key]));
      }
    }
  });
  return string;
...
console.log(replaceEmojis(":potato:"))
